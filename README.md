...
```
### 2. Configuration
Before training, modify the following settings in config.py:
...
| /path/to/mixed_2.wav | /path/to/clean_2.wav | /path/to/noise_2.wav |
...
python train.py
```
During training, the following files will be generated: (1) training_metrics.xlsx: Training metrics for each epoch; (2) training_trends.png: Visualization of training progress; (3) Model checkpoints saved to the paths specified in config.
### 5.Testing
